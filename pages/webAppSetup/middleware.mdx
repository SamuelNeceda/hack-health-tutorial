...
## Intro
In case of need, check out to the branch called `4_middleware`. This branch have already implemented authentication using jwt tokens and also encryption and decryption of the password using bcrypt.

## Next steps
* Current implemention is that when the user is authenticated, the client will have access to the jwt token. Now, everytime the client is making a request to the server to gain access to the private area, the client will have to send the jwt token.
...
* The fubction is going to check the validity of the data received in HTTP requests for user registration and login.
* Create a new file called `inputValidation.ts` and paste the following code inside it:
**We are going to go through this middleware function right after.**
```js filename="jwtGenerator.ts" copy showLineNumbers
...
* We have created a middleware function that checks if the jwt token is valid. This will be used in the next section, when we will implement the private route.
