...
## Intro
In case you have worries that you might not implement everything correctly from the last lecture, you can check out to the branch called `1_server`. If you are goof with your implementation, there is no need to check out to this branch. <br />
...
## Server
For your app, we are going to use Express.js server, which is a minimal and flexible Node.js web application framework. <br />
...
3. In the line 13, we are defining a default port for the server to listen to in case the environment variable `PORT` is not defined. <br />
...
<br />
**Before running following commands, terminate (CTRL + C) the running server in the terminal.** <br />
...
## Summary
By following the tutorial, you should have an Express server running on port 5000 and database with seeded data. Lets move to the next section!
