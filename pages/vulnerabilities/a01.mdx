...
```js filename="jwtAuth.ts" {3} copy
const salt = await bcrypt.genSalt(10);
const bcryptPassword = await bcrypt.hash(password, salt);
```
3. Also do not forget to replace variable ```password``` with ```bcryptPassword```
...
line: ```const validPassword = await bcrypt.compare(password, user.rows[0].password);``` and remove the
line ```const validPassword = password === user.rows[0].password;```.
...
Make sure to include field in payload object is unique to the user and cannot be easily guessed by an attacker.
...
-> Comment out the line ```app.use(cors());``` in ```server.ts``` file and try to login to the HackHealth account.
-> As you can see, the login is not successful and if we open the Developer Tools -> Console, we get the message like:

```
Access to fetch at 'http://localhost:5000/auth/verify' from origin 'http://localhost:3000' has been blocked by CORS policy:
...
```app.use(cors());``` allows requests from any domain, which is a security risk.
**Demo of the attack:**
1. Open Postman app
