...
## Attack
In this demo, we will demonstrate that HackHealth app does not have any prevention from brute force attacks.
<Callout type="info" emoji="ℹ️">
    Form of attack will be dictionary attack, where the attacker tries to guess the password by trying all possible
    combinations from given set.
</Callout>
...
According to an image, you can see parameters which are included in the request and response.
...
During the attack, every password from the list will be tried against the email.
14. After the attack is finished, important information for us are `Status` codes. Also, when clicking on each row with password, we can see in the bottom the request and response of the server.
`Status`:
...
## Prevention
We will implement protection against brute force attacks in the hackHealth app source code using [express-brute](https://www.npmjs.com/package/express-brute) npm package.
<Callout type="info" emoji="ℹ️">
...
After 5 failed attempts, the user will be blocked for 30 seconds and then will be blocked for every failed attempt.
<Callout emoji="💡">
...
Now, all you need to do is to import the `bruteforce` variable and use it as middleware in the `src/server/src/routes/jwtAuth.ts` file and add it to the `login` route.
...
## Verification
Try to again start an attack with the same properties as before. In the attack progress, you should see that only first five request were processed and the rest were blocked.
