...
## Attack
### Code preparation
To prepare for this attack, we need to slightly modify the implementation of handling the login request, because we have already implemented secure way in section [❕JWT authentication](/cybersecurity/a02/jwt). <br />
...
* When you open Developer Tools, in the Console, when digging deeper into `user.rows` in JSON response from the server, you will see the type of the database which is used. <br />
<Image src="/a03/a03_1.png" alt="Burp intruder" width={1000} height={300}/>
#### **Database name** <br />
* Enter the following inside the email field on HackHealth login screen, keep the password field empty and hit Login.
```sql copy
' UNION SELECT current_database(), null--
```
* After digging deeper into servers response, in `user.rows`, you will see the name of the database: `hackhealthdb` <br />
#### **Names of tables** <br />
...
* Now, when you try to log in with the credentials you have obtained in the previous step, you will not be able to log in -> the table of users just do not exist anymore. <br />
<Callout type="info" emoji="ℹ️">
This was made possible only because when creating the tables, foreign key were created with CASCADE option. It means that when the table is dropped, all the tables that depend on it are also dropped. <br />
</Callout>
...
<Callout type="error" emoji="️🚫">
  Do not forget to put the database to initial state after the attack. <br />
...
In the source code, we can replace current implementation of handling login request with the following code:
```js filename="jwtAuth.ts"  copy showLineNumbers
// LOGIN
...
<br />
Also, in the file `Login.js` (easily find by pressing Shift+Shift and typing Login.js), we need to delete line:
```js copy
console.log(parseJSON)
```
By deleting the line, we will no longer be able to see the response from the server in the Console in Developer Tools, which is definitely a better practise.


## Verification
To verify that the vulnerability has been fixed, you can try running some commands from Attack section again. The result should be, that injecting malicious SQL code into email field will not trigger any unintended behavior. <br /> 
